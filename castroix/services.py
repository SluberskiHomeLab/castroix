"""
Media service definitions and launching logic.
"""
import webbrowser
import subprocess
import platform
...
class MediaService:
    ...
    def __init__(self, name: str, url: Optional[str] = None, 
        ...
        Args:
            ...
            command: Shell command to execute (optional, takes precedence over URL)
            icon_color: Hex color code for the button background
            icon_file: Path to PNG icon file (optional)
        """
        self.name = name
        self.url = url
    ...
    def _get_browsers_for_platform(self, system: str) -> List[Tuple[str, str]]:
        ...
        Args:
            system: Platform system name (Windows, Darwin, Linux)
            
        Returns:
            List of (browser_name, command_template) tuples
        """
        if system == "Windows":
...
class ServiceManager:
    """Manages collection of media services"""
    ...
    def __init__(self, services_config: dict):
        ...
        """
        self.services = []
        for key, service_config in services_config.items():
    ...
    def get_services(self) -> List[MediaService]:
        """Get list of all services
        
        Returns:
            List of MediaService objects
        """
